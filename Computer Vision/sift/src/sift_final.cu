...
/*  	n、m：图像高、宽
/*  	feats：用于存储提取出来的sift特征的vector容器
/********************************************************************/
...
extern "C" void test(gray_t* gray_img, int n, int m, gray_t* res_img, int* res_n, int* res_m, int res_num, int S, double sigma_init) {
    ...
    for (int gi = 0; gi < g_size && gi < res_num; ++gi) {
        res_n[gi] = Gauss_pyramid[gi].n;
        res_m[gi] = Gauss_pyramid[gi].m;
        for (int i = 0; i < res_n[gi] * res_m[gi]; ++i) {
            res_img[i] = Gauss_pyramid[gi].img[i];
        }
...
/*	函数描述：进行第一维卷积
/********************************************************************/
__global__ void conv1(const gray_t* img_src, gray_t* pre_filter, gray_t* filter, gray_t* temp_res, int n, int m, int mid, int loop){  
...
/*	函数描述：进行第二维卷积
/********************************************************************/
__global__ void conv2(const gray_t* temp_res, gray_t* pre_filter, gray_t* filter, gray_t* img_dst, int n, int m, int mid, int loop) {  
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int i = index / m;
    int j = index % m;
    extern __shared__ gray_t filter_share[];
    ...
    if (i < n && j < m) {
        ...
        for (int yj = j_sta; yj <= j_end; ++yj) {
            ...
            temp += filter_share[mid - myabs(j - yj)] * temp_res[i * m + yj];
        }
        img_dst[pos] = temp;
    }
}
...
/*  	sigma：高斯模糊参数
/********************************************************************/
void guassian_smooth(const gray_t* device_img_src, gray_t** img_dst_ptr, int n, int m, double sigma) {
    ...
    gray_t* device_pre_filter; // pre_filter[i]表示sum(filter[0], ..., filter[i]);
    checkCudaErrors(cudaMalloc((void **) &device_filter, sizeof(gray_t) * (mid + 1)));// 因为高斯卷积核的对称性 所以只存储前一半加一个参数
    ...
    if (mid > 256) {
        ...
        filter = new gray_t[mid + 1];
        pre_filter = new gray_t[mid + 1]; // pre_filter[i]表示sum(filter[0], ..., filter[i])
        double total = 0;
...
/******************************************************************
/*	函数: half_sample_gpu
/*	函数描述:对图像进行1/2下采样的gpu实现
/*	参数描述:
/*		img_src：原图像地址
/*		img_dst：保存下采样图像的地址
/*  	nn、nm：图像高、宽，更新为下采样图像的大小
/********************************************************************/
...
void build_Gauss_pyramid(gray_t* gray_img, int n, int m, std::vector<Layer>& Gauss_pyramid, int S, double sigma_init) {
    ...
    for (int octave = 0; n >= 3 && m >= 3; ++octave) {
        if (octave == 0) {
            ...
            gray_t* device_gray_img;
            checkCudaErrors(cudaMalloc((void **) &device_gray_img, sizeof(gray_t) * n * m));
            checkCudaErrors(cudaMemcpy(device_gray_img, gray_img, sizeof(gray_t) * n * m, cudaMemcpyHostToDevice));
            guassian_smooth(device_gray_img, &cur_img, n, m, sigma);
            delete[] gray_img; // 删除上采样图像
            checkCudaErrors(cudaFree(device_gray_img));
            gray_img = nullptr;
...
/**
/*	函数: diveup
/*	函数描述: 返回 total / threadnum 的向上取整结果
/*	参数描述:
/*		total
/*		threadnum
 */
